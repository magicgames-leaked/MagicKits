...
class CategorySelectForm {
    ...
    public static function sendTo(Player $player) : void {
        ...
        foreach(CategoryManager::getAll() as $category) {
            if($category->hasPermission($player)) {
                $ui->addButton(LangUtils::getMessage("category-unlocked-format", true, ["{NAME}" => $category->getName()]), 1, "https://i.imgur.com/g6AiAM0.png", $category->getName());
            } elseif(DataManager::getKey(DataManager::CONFIG, "show-locked-categories")) {
                $ui->addButton(LangUtils::getMessage("category-locked-format", true, ["{NAME}" => $category->getName()]), 1, "https://i.imgur.com/Z8EjrUa.png", $category->getName());
            }
        }
